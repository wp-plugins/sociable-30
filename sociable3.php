...
class Sociable {
    ...
    function Sociable() {
        ...
        // $this->pro_button = '<a href="https://www.e-junkie.com/ecom/gb.php?i=782291&c=single&cl=128716" target="ejejcsingle">'
        //  . '<img style="padding-right:25px;" alt="Download Pro" src="' . plugins_url('/images/gopro.png', __FILE__) . '"/>'
        //  . '</a>';

        // $this->pro_link = '<a href="https://www.e-junkie.com/ecom/gb.php?i=782291&c=single&cl=128716" target="ejejcsingle">'
        //  . '<img style="vertical-align:middle;" alt="Download Pro" src="' . plugins_url('/images/smallpro.png', __FILE__) . '"/>'
        //  . '</a>';
    ...
    function wp_print_styles_hook() {
        $options = get_option('sociable');
        wp_enqueue_style('sociable3', plugins_url('sociable.css', __FILE__));
        wp_enqueue_style('sociable3-custom', plugins_url('sociable_custom.css', __FILE__));
    }
    ...
    function content_hook($content='') {
        ...
        if ($this->debug) {
            ...
            $options = get_option('sociable');
            if ($options)
                echo "content_hook: options = " . print_r($options, true);
            echo " -->";
        }
    ...
    function get_links($display = null, $tagline = null) {
        ...
        if ($tagline) {
            $html .= "<div class='sociable-tagline'>";
            $html .= stripslashes($tagline);
            $html .= "</div>";
        }

        if (method_exists($this, 'get_facebook_iframe') && !is_feed())
            $html .= "<div>" . $this->get_facebook_iframe($permalink) . "</div>";
    ...
    function show_site($sitename, $iconurl, $url=null, $options=null) {
        ...
        $link_title = ($sitename == 'Add to favorites') ? "title='$description'" : "";
        ...
        $html = "<a rel='nofollow' $link_target $link_title href='$url'>"
